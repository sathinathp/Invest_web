...
app.post('/api/pitch', upload.single('pitchAudio'), (req, res) => {
    try {
        ...
        if (audioFile && !finalSpeechText) {
            finalSpeechText = `[Simulated Audio Pitch Transcription]: Hello, my name is ${founderName} from ${startupName}. We are building in the ${sector} sector. Our product solves the following problem: ${problem}. We believe now is the perfect time because: ${whyNow}. We hope to partner with Antigravity Ventures.`;
        }
